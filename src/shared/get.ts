import { rgbToLinearRgb } from '../models/rgb';
 ...
 */
export const getBrightness = (rgb: RgbColor): number => {
 ...
 */
export const getContrast = (rgb1: RgbColor, rgb2: RgbColor): number => {
 ...
 */
export function getDeltaE2000(color1: LabColor, color2: LabColor): number {
  ...
  const deltaE =
    ((dL / kl / sL) ** 2 + (dC / kc / sC) ** 2 + (dH / kh / sH) ** 2 + (Rt * dC * dH) / (kc * sC * kh * sH)) ** 0.5;
